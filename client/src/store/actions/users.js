...
export const registerUser = createAsyncThunk(
    ...
    async({email,password},{dispatch})=>{
        try{
            ...
            //this where error live in axios
...
export const signInUser = createAsyncThunk(
    ...
    async({email,password},{dispatch})=>{
        try{
            ...
            dispatch(successGlobal('Welcome back, we have been missing you!'))
            return { data:request.data.user, auth:true }          
        } catch(error) {
             dispatch(errorGlobal(error.response.data.message))
             throw error;
...
export const signOut = createAsyncThunk(
    'users/signOut',
    async()=>{
        //no try n catch so always go to fullfilled
        removeTokenCookie();
    }
)
...
export const changeEmail = createAsyncThunk(
    ...
    async(data, {dispatch})=>{
        try{
            ...
            dispatch(successGlobal('email has been updated!'))
            return {
...
export const accountVerify = createAsyncThunk(
    ...
    async(token, {dispatch,getState})=>{
        try{
            ...
            dispatch(successGlobal('Account has been verified !'))
